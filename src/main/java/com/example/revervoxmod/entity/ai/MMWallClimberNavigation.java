...
public class MMWallClimberNavigation extends MMPathNavigateGround {
    @Nullable
    private BlockPos pathToPosition;
    public MMWallClimberNavigation(RevervoxGeoEntity entity, Level level) {
        super(entity, level);
    }
    ...
    public void tick() {
        if (!this.isDone()) {
            ...
            if (this.pathToPosition != null) {
                if (!this.pathToPosition.closerToCenterThan(this.mob.position(), Math.max(this.mob.getBbWidth(), 1.0D)) && (!(this.mob.getY() > (double)this.pathToPosition.getY()) || !(BlockPos.containing(this.pathToPosition.getX(), this.mob.getY(), this.pathToPosition.getZ())).closerToCenterThan(this.mob.position(), Math.max(this.mob.getBbWidth(), 1.0D)))) {
